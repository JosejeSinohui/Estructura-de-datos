...
public class Imprimir {
    ...
    private static void printNodoInternal(List<Nodo> nodes, int level, int maxLevel) {
        ...
        for (Nodo node : nodes) {
            if (node != null) {
                ...
                newNodos.add(node.subArbolIzq());
                newNodos.add(node.subArbolDer());
            } else {
                newNodos.add(null);
                newNodos.add(null);
                System.out.print("\t");
        ...
        for (int i = 1; i <= endgeLines; i++) {
            for (int j = 0; j < nodes.size(); j++) {
                ...
                if (nodes.get(j).subArbolIzq() != null) {
                    System.out.print("/");
                } else {
                ...
                if (nodes.get(j).subArbolDer() != null) {
                    System.out.print("\\");
                } else {
                    Imprimir.printWhitespaces(1);
                }
    ...
    private static int maxLevel(Nodo node) {
        ...
        return Math.max(Imprimir.maxLevel(node.subArbolIzq()), Imprimir.maxLevel(node.subArbolDer())) + 1;
    }
